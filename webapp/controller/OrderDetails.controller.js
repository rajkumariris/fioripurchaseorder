...
], function (BaseController, Filter, JSONModel, MessageToast, Fragment, MessageBox) {
    "use strict";
    return BaseController.extend("demo.controller.OrderDetails", {
        ...
        _onSavePressed: async function (oEvent) {
            ...
            try {
                ...
                let sErrorMessage = "Failed to update Purchase Order";
                let aErrorDetails = [];
                ...
                if (oError.responseText) {
                    try {
                        ...
                        const sResponseText = oError.responseText.trim();
                        ...
                        if (sResponseText.startsWith("{")) {
                            ...
                            if (oResponse.error && oResponse.error.message) {
                                if (typeof oResponse.error.message === "object" && oResponse.error.message.value) {
                            ...
                            if (oResponse.error && oResponse.error.innererror && oResponse.error.innererror.errordetails) {
                                aErrorDetails = oResponse.error.innererror.errordetails.map(err => {
                                    return "• " + err.message;
                                });
                            }

                        } else if (sResponseText.startsWith("<")) {
                            // XML format
                            const parser = new DOMParser();
                            const xmlDoc = parser.parseFromString(oError.responseText, "application/xml");

                            // Extract message from XML
                            const messageElement = xmlDoc.getElementsByTagName("message")[0];
                            if (messageElement) {
                                sErrorMessage = messageElement.textContent;
                            }

                            // Extract code if needed
                            const codeElement = xmlDoc.getElementsByTagName("code")[0];
                            if (codeElement) {
                                aErrorDetails.push("• Error Code: " + codeElement.textContent);
                            }

                        } else {
                            // Plain text format
                            sErrorMessage = oError.responseText || oError.message;
                        }
                ...
                // Build final error message with all details
                ...
                MessageBox.error(sFinalErrorMessage, {
                    title: "Update Failed",
                    onClose: function () {
                        // Keep edit mode enabled so user can fix and retry
                    }
                });
            }
